...
class Logger:
    ...
    def log(self, message, level="info"):  # Changed default level to "info"
        ...
        if level == "success":
            ...
            self.console.print(
                f"\n[info]{message}[/]"
            )  # Changed to "info" to use blue color
...
class RedeEletricaPandaPower:
    ...
    def loading_networks_cases(self, network_name="14"):
        ...
        match network_name:
            case "3":
                self.network_name = "Simple 3 Bus Case"
                network = pp.create_empty_network()
                b1 = pp.create_bus(net=network, vn_kv=110.0, name="Bus 1")
                b2 = pp.create_bus(net=network, vn_kv=110.0, name="Bus 2")
                b3 = pp.create_bus(net=network, vn_kv=110.0, name="Bus 3")
                pp.create_ext_grid(net=network, bus=b1, vm_pu=1.02, name="Grid")
                pp.create_line(
                    net=network,
                    from_bus=b1,
                    to_bus=b2,
                ...
                pp.create_load(net=network, bus=b3, p_mw=30, q_mvar=10, name="Load")

            case "5":
                self.network_name = "Case 5"
                network = pw.case5()

            case "9":
                self.network_name = "Case 9"
                network = pw.case9()
            ...
            case _:
                print(
                    "Rede não encontrada, forneça o numero como string como: IEEE 14 = '14'"
                )
                print(
    ...
    def criar_mapeamento_ramos(self):
        ...
        self.mapeamento_ramos = {"linhas": {}, "trafos": {}}
    ...
    def validar_dados(self, df_agendamento, df_contingencia):
        ...
        for _, row in df_agendamento.iterrows():
            ...
            if not (
                ramo in self.mapeamento_ramos["linhas"]
                or ramo in self.mapeamento_ramos["trafos"]
            ):
    ...
    def hashtableindex(
        ...
        return (
            (k * (n_carregamentos) * (n_contingencias))
            + ((carregamento - 1) * (n_contingencias))
            + (contingencia - 1)
        )
    ...
    def plot_rede_eletrica_status(self, trafo_status, line_status):
        ...
        legend_handles = [
            bars[0],
            bars[-1] if not all_status[0] else bars[all_status.index(False)],
        ]
    ...
    def plot_trafo_status(self, net):
        ...
        fig, ax = plt.subplots(figsize=(10, 6))  # Ajustar o tamanho conforme necessário
        bars = ax.bar(
            x_labels, loading_percent, color="b"
        )  # Barras azuis para loading_percent
        ...
        ax.set_xticks(x_labels)  # Definir os ticks do eixo x
        ax.legend(
            [bars[0], bars[-1]], ["Ligado", "Desligado"], loc="best"
        )  # Adicionar legenda
    ...
    def plot_potencia_ativa_reativa(self):
        ...
        # === Gráfico 1: Potências ===
        fig_pot = make_subplots(
            rows=1, cols=1, subplot_titles=["Potências Ativa e Reativa"]
        )
        ...
        if not net.load.empty:
            ...
            fig_pot.add_trace(
                go.Scatter(
                    x=idx, y=net.res_load["p_mw"], name="P Cargas (MW)", mode="lines"
                )
            )
            fig_pot.add_trace(
                go.Scatter(
                    x=idx,
                    y=net.res_load["q_mvar"],
                    name="Q Cargas (MVAr)",
                    mode="lines",
                )
            )
        fig_pot.update_layout(
            title="Gráfico de Potências", xaxis_title="Índice", yaxis_title="Potência"
        )
        ...
        fig_tens.add_trace(
            go.Scatter(
                x=idx_bus, y=net.res_bus["vm_pu"], name="Tensão Barras", mode="lines"
            )
        )
        if not net.line.empty:
            ...
            fig_tens.add_trace(
                go.Scatter(
                    x=idx_line,
                    y=net.res_line["vm_from_pu"],
                    name="Tensão Linhas",
                    mode="lines",
                )
            )
        fig_tens.update_layout(
            title="Gráfico de Tensões", xaxis_title="Índice", yaxis_title="Tensão (pu)"
        )
    ...
    def plot_all_results_to_html(self, output_file="relatorio_eletrico.html"):
        ...
        if not net.line.empty:
            fig_corr.add_trace(
                go.Bar(
                    x=np.arange(len(net.res_line)),
                    y=net.res_line["i_ka"],
                    name="Corrente Linhas (kA)",
                )
            )
        if not net.trafo.empty:
            fig_corr.add_trace(
                go.Bar(
                    x=np.arange(len(net.res_trafo)),
                    y=net.res_trafo["i_hv_ka"],
                    name="Corrente Trafos (kA)",
                )
            )
        fig_corr.update_layout(
            title="Gráfico de Correntes",
            xaxis_title="Índice",
    ...
    def plot_power_data(self, index=None):
        ...
        """
        print(
            "Tentativa do primeiro plot com tensões e potencias de linhas, trafos e barras"
        )
        try:
            ...
            if not self.net.line.empty:
                ...
                ax.plot(
                    index_line,
                    self.net.res_line["p_from_mw"],
                    label="P Ativa Linhas (MW)",
                    color="blue",
                )
                ax.plot(
                    index_line,
                    self.net.res_line["q_from_mvar"],
                    label="Q Reativa Linhas (MVAr)",
                    color="red",
                )
                ax.plot(
                    index_line,
                    self.net.res_line["vm_from_pu"],
                    label="Tensão Linhas (pu)",
                    color="cyan",
                )
            ...
            if not self.net.trafo.empty:
                ...
                ax.plot(
                    index_trafo,
                    self.net.res_trafo["p_hv_mw"],
                    label="P Ativa Trafos (MW)",
                    color="green",
                )
                ax.plot(
                    index_trafo,
                    self.net.res_trafo["q_hv_mvar"],
                    label="Q Reativa Trafos (MVAr)",
                    color="orange",
                )
            ...
            if not self.net.load.empty:
                ...
                ax.plot(
                    index_load,
                    self.net.res_load["p_mw"],
                    label="P Carga (MW)",
                    color="darkgreen",
                )
                ax.plot(
                    index_load,
                    self.net.res_load["q_mvar"],
                    label="Q Carga (MVAr)",
                    color="brown",
                )
        ...
        for horario in range(horas):
            if horario == 0:  # Condição inicial
                if matriz_horas[horario] > 0:
                    ...
                    matriz_cenarios.append(
                        [perfil] + matriz_desligamentos_horas[:, horario].tolist()
                    )
            else:
                if (
                    matriz_horas[horario] != matriz_horas[horario - 1]
                    and matriz_horas[horario] > 0
                ):
                    # Nova topologia
                    perfil = self.calcular_perfil(horario, ls, le, ms, me, hs, he)
                    matriz_cenarios.append(
                    ...
                    if (
                        horario - 1 in [ms, hs] and matriz_cenarios
                    ):  # Verifica se matriz_cenarios não está vazia
                        perfil = self.calcular_perfil(horario, ls, le, ms, me, hs, he)
    ...
    def executar_fluxo_de_potencia(self, fast=False):
        """
        ...
        try:
            if fast:
                self.log(
                    "Executando fluxo de potência com numba-python para acelerar o processo...",
                    level="info",
                )
        ...
        for i, estado in enumerate(estados):
            if estado == 1:  # Verifica se o ramo deve ser desligado
                ...
                for k in range(len(self.net.line)):
                    ...
                    if (
                        self.net.line["from_bus"][k] == ramo[0]
                        and self.net.line["to_bus"][k] == ramo[1]
                    ) or (
                        self.net.line["from_bus"][k] == ramo[1]
                        and self.net.line["to_bus"][k] == ramo[0]
                    ):
                        linhas_desligar.append(
                            ramo
                        )  # Adiciona o ramo à lista de linhas

                for k in range(len(self.net.trafo)):
                    if (
                        self.net.trafo["hv_bus"][k] == ramo[0]
                        and self.net.trafo["lv_bus"][k] == ramo[1]
                    ) or (
                        self.net.trafo["hv_bus"][k] == ramo[1]
                        and self.net.trafo["lv_bus"][k] == ramo[0]
                    ):
                        trafos_desligar.append(
                            ramo
                        )  # Adiciona o ramo à lista de trafos
    ...
    def desligar_elementos(self, linhas_desligar, trafos_desligar):
        ...
        if not linhas_desligar:
            ...
            for l in linhas_desligar:
                ...
                index_linha = self.net.line.loc[
                    (self.net.line["from_bus"] == l[0])
                    & (self.net.line["to_bus"] == l[1])
                ].index
        ...
        if not trafos_desligar:
            ...
            for t in trafos_desligar:
                ...
                # Encontra o índice da linha com base em from_bus e to_bus
                index_trafo = self.net.trafo.loc[
                    (self.net.trafo["hv_bus"] == t[0])
                    & (self.net.trafo["lv_bus"] == t[1])
                ].index
    ...
    def calcular_potencia_aparente_trafos(self):
        ...
        if not self.net.res_trafo.empty:
            ...
            potencia_high_tensao = (
                self.net.res_trafo["p_hv_mw"] ** 2
                + self.net.res_trafo["q_hv_mvar"] ** 2
            ) ** 0.5
            potencia_baixa_tensao = (
                self.net.res_trafo["p_lv_mw"] ** 2
                + self.net.res_trafo["q_lv_mvar"] ** 2
            ) ** 0.5
    ...
    def calcular_potencia_aparente_linhas(self):
        ...
        return (
            self.net.res_line["p_from_mw"] ** 2 + self.net.res_line["q_from_mvar"] ** 2
        ) ** 0.5
    ...
    def imprimir_resultados(self):
        ...
        high_power_transformador, lower_power_transformador = (
        ...
        dataframe["porcentagem_de_carga_nas_linhas"] = self.net.res_line[
            ["loading_percent"]
        ]
        dataframe["potencia_aparente_nos_transformadores"] = self.net.res_trafo[
            ["p_hv_mw"]
        ]
        dataframe["porcentagem_de_carga_nos_transformadores"] = self.net.res_trafo[
            ["loading_percent"]
        ]
    ...
    def add_barras(self, net, barras):
        for barra in barras:
            pp.create_bus(
                net, name=barra["nome"], vn_kv=barra["tensao"], index=barra["id"]
            )
...
def main_rede_eletrica(simulate=False):
    ...
    if simulate:
        print(
            f"[debug] da classe Model RedeEletricaPandapower =  Simulação {network_modelada.network_name} iniciada"
        )
        network_modelada.ajustar_cargas(
            perfil=2
        )  # Ajusta para o perfil médio (IEEE 14)
        ...
        network_modelada.plot_all_results_to_html(
            output_file=f"./relatorio_eletrico_{network_modelada.network_name}.html"
        )
