        ...
        for cenario in matriz_cenarios:
            ...
            for contingencia_atual in range(num_contingencias):
                ...
                if setupobj.tabela_hash[hash_key] < 0.0:
                    ...
                    # Apenas adiciona à lista se for um cálculo novo
...
def run_simulation_test():
    ...
    params_json = {
        "NUM_GENERATIONS": 40, "CROSSOVER": 0.95, "MUTACAO": 0.25, "POP_SIZE": 5,
        "IND_SIZE": len(horarios_teste), "RCE_REPOPULATION_GENERATIONS": 50,
    ...
    for index, row in agendamento_df.iterrows():
        agendamento_executado.append({
            "ramo": row["ramo"],
            "horario_inicio_solicitado": row["inicio"]
        })

    resultado_final = {
        "fitness": fitness,
        "agendamento_solicitado": agendamento_executado
    }
    ...
    print(json.dumps(resultado_final, indent=4, default=str))
    print("-----------------------------")
