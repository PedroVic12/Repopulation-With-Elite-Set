...
📚 Carreira e Estudos:
    ...
    Estudante de Engenharia Elétrica na UFF, atualmente no 7º período.

    Pesquisa e desenvolve sistemas inteligentes para redes elétricas usando Pandapower, IA e automação.

    Trabalha com Python, Node.js, React, FastAPI, Rust, Flutter, C++ e tecnologias de Machine Learning.

    Criou frameworks de Algoritmos Genéticos e sistemas para agendamento de redes elétricas.

    Está construindo agentes de IA e desenvolvendo projetos de automação residencial com ESP32.

    Tem experiência com deploy em VPS, integração com N8N e sistemas embarcados.

💻 Projetos e Inovações:

    Chatbots para atendimento, apps de delivery e sistemas de agendamento para redes elétricas.

    Frontend dinâmico com Streamlit para visualização de resultados e dashboards.

    Testes avançados com Pandapower para simulação de cenários e otimização de redes elétricas.

    Sistemas backend com WebSocket para integração em tempo real.

    Aplicações em energias renováveis e automação industrial.

🏆 Metas e Propósito:

    Busca ser reconhecido como desenvolvedor fullstack, inovador em IA e engenharia elétrica.

    Está em um processo intenso de transformação pessoal, espiritualidade e foco.

    Objetivo de ficar 30 dias sem maconha para clareza mental e foco nos estudos e projetos.

    Planeja se tornar um desenvolvedor mais disciplinado e focado, com rotina de treino, meditação e estudos.

    Quer criar um legado tecnológico com impacto positivo no mundo, inspirado por Nikola Tesla e Steve Jobs.

❤️ Relacionamentos e Reflexões:

    Recentemente enfrentou desafios emocionais com Danielle, com altos e baixos de confiança.

    Busca estabilidade emocional e equilíbrio em suas relações pessoais.

    Quer focar mais em si mesmo e seus projetos, sem se deixar levar por distrações emocionais.
