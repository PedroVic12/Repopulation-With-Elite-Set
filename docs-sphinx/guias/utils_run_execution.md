# Execução do Framework (`run.py`)

Este script é o ponto de entrada principal para executar o framework de otimização com Algoritmos Genéticos (RCE). Ele orquestra a execução de múltiplas simulações com diferentes configurações de parâmetros, permitindo a realização de estudos de sensibilidade e a comparação de resultados.

## Funcionalidades Principais

-   **Carregamento de Configurações**: Carrega os parâmetros base do `params.json` e as variações de parâmetros do `options.json`.
-   **Geração de Cenários**: Cria todas as combinações possíveis de parâmetros para as execuções.
-   **Seleção da Função Objetivo**: Permite a seleção da função de fitness a ser otimizada, incluindo funções de benchmark e funções específicas para casos de estudo de redes elétricas (IEEE 14, 30, 57, 118 e SIN 45).
-   **Execução do Algoritmo Genético**: Instancia e executa o `AlgoritimoEvolutivoRCE` para cada configuração.
-   **Armazenamento de Resultados**: Salva os resultados de cada execução, incluindo os melhores indivíduos, fitness, e estatísticas de execução, em ficheiros JSON individuais.
-   **Consolidação de Resultados**: Ao final de todas as execuções, invoca um script para consolidar todos os resultados num único ficheiro Excel.
