# dashboard_rce_test_functions.py
...
# --- Funções de Exibição (sem alterações, pois recebem os dados/figura já carregados) ---

def display_summary(data, exec_num):
...
def display_convergence_graph(fig, exec_num):
...
def display_consolidated_results():
    """Verifica e exibe a seção de resultados consolidados,
       buscando o arquivo na pasta FOLDER_NAME."""
...
def main():
    ...
    selected_exec_num = select_execution(execution_numbers)

    if selected_exec_num:
        data, fig = load_execution_data(selected_exec_num)

        if data:
            display_summary(data, selected_exec_num)
            display_statistics_table(data)
            display_convergence_graph(fig, selected_exec_num)
