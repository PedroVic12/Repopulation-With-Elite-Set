...
class DatabaseController:
    ...
    def get_consolidated_data(self):
        """
        Carrega e retorna os dados consolidados do arquivo Excel.
        
        Returns:
            pd.DataFrame: DataFrame com os resultados consolidados ou None se houver erro.
        """
        try:
            # Tenta carregar do arquivo padrão
            if self.consolidated_results_file.exists():
                df = pd.read_excel(self.consolidated_results_file)
                print(f"✅ Dados consolidados carregados: {len(df)} execuções")
                return df
            else:
                # Fallback para o caminho hardcoded (mantido para compatibilidade)
                fallback_path = "/home/pedrov12/Documentos/GitHub/Repopulation-With-Elite-Set/src/output/resultados_consolidados.xlsx"
                if Path(fallback_path).exists():
                    df = pd.read_excel(fallback_path)
                    print(f"✅ Dados consolidados carregados (fallback): {len(df)} execuções")
                    return df
                else:
                    print("⚠️ Arquivo de resultados consolidados não encontrado")
                    return None
        except Exception as e:
    ...
    def run_consolidation(self):
        ...
        Este método:
        1. Busca todos os arquivos de resultado individuais
        2. Consolida os dados em um DataFrame único
        3. Salva o resultado em Excel com formatação
        4. Atualiza o arquivo de resultados consolidados
        ...
        try:
            ...
            if success:
                ...
                # Atualiza o arquivo de resultados consolidados na instância
                self.consolidated_results_file = self.output_dir / "resultados_consolidados.xlsx"
    ...
    def get_consolidation_status(self) -> dict:
        ...
        try:
            ...
            if self.consolidated_results_file.exists():
                ...
                file_info = self.get_file_info("resultados_consolidados.xlsx")
                if file_info:
                ...
                # (se há mais arquivos individuais que execuções no consolidado)
