...
class NetworkCanvas(FigureCanvas):
    ...
    def plot_network(self, net):
        """Plots the network, highlighting out-of-service elements."""
        self.ax.clear()
        ...
        try:
            ...
            if not oos_lines.empty:
                lc = plot.create_line_collection(net, lines=oos_lines, color="r", linestyle="--")
        ...
        self.draw()
        # Force close any extra figures pandapower might create
        plt.close('all')
...
class SidebarWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.element_list.itemChanged.connect(self._on_contingency_selection_changed)
        self.run_button.clicked.connect(self.run_simulation_requested.emit)

    def _on_contingency_selection_changed(self):
        contingencies = []
